...
}> = Object.values(JUSTIFY_CONTENT).map(unit => ({ label: unit, value: unit }));
export const Alignment: FC = () => {
  const alignItems = useRecoilValue(selectHighlightedNodeGridPropState('alignItems'));
  const justifyContent = useRecoilValue(selectHighlightedNodeGridPropState('justifyContent'));
  const highlightedNode = useRecoilValue(selectHighlightedNodeState);
  const [grid, setGridState] = useRecoilState(gridState);
  const { gray100_dark400 } = useThemeColors();

  const onSelectUnit = (v: ALIGN_ITEMS | JUSTIFY_CONTENT, alignment: 'alignItems' | 'justifyContent') => {
  ...
  return (
    <Box p={4}>
      <Flex gap={4}>
        <Box flex={1}>
          <Heading variant="medium-title" mb={4}>AlignItems</Heading>
        ...
        <Box flex={1}>
          <Heading variant="medium-title" mb={4}>JustifyContent</Heading>
          <Menu variant="base" placement="bottom-end" matchWidth>
            <MenuButton
              as={Button}
              w="full"
